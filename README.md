# VITA-49 RF Chaos Workbench — Rust v0.5.3

A native Rust VITA-49/VRT chaos-engineering workstation built around the **real RS-34 Bx01 stream on RS5**. v0.5.3 hardens and visually rebuilds the prototype dashboard with the selected **Operator Workstation** interface and removes the production synthetic/demo packet source.

The design reference is preserved at `docs/operator_workstation_reference.png`.

## What v0.5.3 means by “real data”
...
There is intentionally **no production `demo`/synthetic-source subcommand** in v0.5.3. Synthetic packets remain only in Rust unit tests, where deterministic fixtures are necessary to test parsing/DSP/fault behavior.
...
The artifact-generation environment used to prepare v0.5.3 does not contain `cargo`/`rustc`, so the release binary could not be compiled here. The package therefore includes static validation and makes **RS5 `cargo test` + `cargo build --release` the authoritative gate**. See [`BUILD_VALIDATION.md`](BUILD_VALIDATION.md).
